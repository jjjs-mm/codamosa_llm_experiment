...
print(">>> DEBUG: Pynguin Generator is starting...")
...
def _run() -> ReturnCode:
    ...
    from pathlib import Path
    p_path = config.configuration.project_path
    print(f">>> [DEBUG] Internal Path: '{p_path}'")
    print(f">>> [DEBUG] Path Exists? {os.path.exists(p_path)}")
    
    # 这一行是关键：检查 Pynguin 是否觉得这个路径下少了一些特定的 marker
    # 比如 Pynguin 可能会检查 p_path 是否包含 "__init__.py" 或者是特定的目录结构
    
    setup_result = _setup_and_check()
    if setup_result is None:
        print(">>> [DEBUG] _setup_and_check failed!")
        # 强制打印出当前 Pynguin 看到的所有配置，看看路径是不是被拼接错了
        print(f">>> [DEBUG] Full Config: {config.configuration}")
...
def _export_test_cases(
    ...
    return target_file
def main():
    print(">>> [STEP 1] Entered main()")  # 注入点 1
    
    # 寻找解析参数的地方，通常长这样：
    # args = parse_args() 
    # 或者
    # parser = PynguinArgumentParser()
    # args = parser.parse_data()

    print(">>> [STEP 2] Starting Argument Parsing...") # 注入点 2
    
    # ... 原有的解析代码 ...
    
    print(">>> [STEP 3] Arguments parsed successfully") # 注入点 3
    
    # 寻找设置配置的地方
    # set_configuration(args)
    
    print(">>> [STEP 4] Configuration set") # 注入点 4

    # 寻找启动执行的地方
    # run_pynguin(args)
    
    print(">>> [STEP 5] About to run algorithm...") # 注入点 5
...
if __name__ == "__main__":
    ...
    print(">>> [TRACE] Script Entry Point")
    try:
        parser = cli._create_argument_parser()
        # 解析参数
        args, _ = parser.parse_known_args(sys.argv[1:])
        
        # 【核心修正】从 args.config 中提取并覆盖全局配置
        if hasattr(args, 'config'):
            new_config = args.config
            # 直接将解析出的 Configuration 对象赋值给全局单例
            config.configuration = new_config
            
            print(f">>> [DEBUG] SUCCESS! Injected Path: '{config.configuration.project_path}'")
            print(f">>> [DEBUG] SUCCESS! Injected Module: '{config.configuration.module_name}'")
        else:
            print(">>> [ERROR] Could not find 'config' in parsed arguments!")

        # 运行主逻辑
        result = _run()
        print(f">>> [TRACE] Finished with result: {result}")
        
    except Exception as e:
        print(">>> [TRACE] CRITICAL ERROR DURING CONFIG SYNC:")
        traceback.print_exc()
        sys.exit(1)