...
export INPUT_DIR="$ROOT_DIR/final_mutmut_suite_sanic/stage2_v3"
export OUTPUT_DIR="$ROOT_DIR/final_mutmut_suite_sanic/stage3_cascade"
...
echo "📂 [物理隔离] 同步 Sanic Stage 2 精英用例至 Stage 3 级联专属沙箱..."
mkdir -p "$OUTPUT_DIR"
rm -f "$OUTPUT_DIR"/*.py 
...
echo "🧠 [级联引擎点火] 启动 r1_incremental_booster.py 强攻 sanic..."
echo "----------------------------------------------------------------"
...
echo "🎉 sanic 满血版 Stage 3 级联增强全部完毕！"