...
class _OpenAILanguageModel:
    ...
    def _call_mutate(self, function_to_mutate: str) -> str:
        ...
        # 1. 修正 URL 为传进来的相对路径或直接写死 DeepSeek
        ...
        # 4. 适配 Chat Completions 的解析结构 (原先是 res.json()["choices"][0]["text"])
    ...
    def _call_completion(
        ...
        try:
            if "choices" in res_data and len(res_data["choices"]) > 0:
                ...
                if "message" in choice:
                    result_text = choice["message"].get("content", "")
                else:
                    result_text = choice.get("text", "")
            else:
                result_text = ""
        except Exception as e:
            logger.error("解析 LLM 响应出错: %s", e)
            return ""

        # 防止 DeepSeek 极小概率返回 JSON null
        ...
        if "```" in result_text:
            ...
            if len(parts) >= 3:
                code_block = parts[1]
                if code_block.strip().lower().startswith("python"):
                    code_block = code_block.strip()[6:]
