...
# 专门针对 Python 3.10 的枚举补丁
class FallbackCompare(enum.IntEnum):
    IN = 6
    NOT_IN = 7
    IS = 8
    IS_NOT = 9
...
class BranchCoverageInstrumentation(InstrumentationAdapter):
    ...
    def _instrument_compare_based_conditional_jump(
        ...
        # [补丁] Python 3.10 的 bytecode 库移除了这些枚举值。
        # 我们直接使用它们在底层字节码映射中的经典整数值，绕过 AttributeError。
        _IN, _NOT_IN, _IS, _IS_NOT = 6, 7, 8, 9
        ...
        match operation.name:
            ...
            case "IS_OP":
                ...
                compare = _IS_NOT if operation.arg else _IS
            case "CONTAINS_OP":
                compare = _NOT_IN if operation.arg else _IN
            case _:
