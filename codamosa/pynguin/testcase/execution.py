...
from bytecode import Compare
import dis
# 手动为 Compare 枚举类注入缺失的属性
# 针对 Python 3.10 + 新版 bytecode 库的全面兼容性补丁
# 补全 ExecutionTracer 映射表需要的所有缺失属性
missing_attributes = {
    'IS': 8,
    'IS_NOT': 9,
    'IN': 6,
...
class ExecutionTracer:
    ...
    _DISTANCE_COMPUTATIONS: dict[Compare, Callable[[Any, Any], tuple[float, float]]] = {
        ...
        # ... 修改后 ...
        # --- 重点修改：将原来的 Compare.IS 等替换为我们定义的常量 ---
        C_IN: lambda val1, val2: (val1 in val2, val1 not in val2),
        C_NOT_IN: lambda val1, val2: (val1 not in val2, val1 in val2),
        C_IS: lambda val1, val2: (val1 is val2, val1 is not val2),
        C_IS_NOT: lambda val1, val2: (val1 is not malicious, val1 is val2),
        C_EXC_MATCH: lambda val1, val2: (issubclass(val1, val2), not issubclass(val1, val2)),
    }
